...
| **이미지 (업로드·회원가입·생명주기)** | **(1) 미리 업로드**: 파일 처리를 비동기화해 DB 트랜잭션 점유를 줄이고, 본문 작성 실패 시에도 리소스 재사용이 가능한 구조로 두었습니다.<br>**(2) 회원가입·signupToken**: 가입 전에는 계정이 없어 프로필 이미지는 uploader_id 없이 저장됩니다. 가입 시 해당 이미지를 이 사용자 소유로 묶을 때, 업로드 시에만 발급된 **signupToken**으로 검증해, imageId만 아는 제3자가 남의 이미지를 가로채는 것을 막고 소유 증명을 합니다.<br>**(3) 생명주기(Reference Counting)**: ref_count로 참조 수를 관리하고, 0이 되면 삭제하는 즉시 처리 방식을 쓰며, 향후 비동기 배치(Batch GC) 전환 확장을 고려했습니다. |
...
- **JWT 도입**: 규모가 커지거나 모바일 앱 개발로 확장 및 트래픽 증가 시, **JWT Access Token + Redis(블랙리스트/토큰 스토어)** 조합으로 전환해 DB 부하를 줄이고, 무상태(Stateless)에 가까운 확장성을 확보할 수 있습니다.
- **세션/토큰 저장소**: 단일 인스턴스 기준에서는 MySQL로 충분하지만, 멀티 인스턴스·멀티 리전에선 **Redis 클러스터**를 세션/토큰 저장소로 사용하여 일관성 있는 인증 상태를 유지합니다.
