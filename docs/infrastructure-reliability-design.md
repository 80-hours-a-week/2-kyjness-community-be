...
| **WAS** | FastAPI, Gunicorn + Uvicorn worker | `Dockerfile`: `-w 4`, `UvicornWorker`; `app/main.py` 진입 |
...
| **스토리지** | 로컬 또는 S3 | `app/core/storage.py`: `STORAGE_BACKEND=local|s3`, S3 시 boto3 사용 |
...
graph TD
    ...
    subgraph Data_Plane["데이터 영역"]
        RDS_Primary[(RDS MySQL<br/>Primary AZ-a)]
        RDS_Replica[(RDS Read Replica<br/>AZ-b)]
    ...
    ASG2 --> Redis
    ASG2 --> S3
    RDS_Primary -.->|동기 복제| RDS_Replica
    ASG1 -.->|Read 전용| RDS_Replica
    ASG2 -.->|Read 전용| RDS_Replica
```
...
| 5 | **RDS MySQL Primary** | 쓰기·트랜잭션, Multi-AZ 배포 시 동기 복제로 장애 시 자동 페일오버 |
| 6 | **RDS Read Replica** | 읽기 부하 분산(피드·조회), 주 DB 장애 시 수동/자동 전환 가능 |
| 7 | **ElastiCache Redis** | 세션 저장, 인기 게시글/피드 캐시, Rate Limit 카운터 |
| 8 | **S3** | 업로드 이미지, 정적 웹 자산(선택), 버전·수명 주기로 백업 |
...
sequenceDiagram
    ...
    WAS->>S3: PutObject
    S3-->>WAS: 성공
    WAS->>RDS: Primary에 메타 저장
    WAS-->>C: 201 Created
```

### 1.5 현재 코드·설정 기준 구현 상태 (코드 반영)
...
| **헬스체크** | `app/main.py` L89–101 | `GET /health` → `check_database()` 호출. 성공 시 200 + `{"status":"ok","database":"connected"}`, 실패 시 **503** + `database: disconnected`. ALB 타겟 그룹 헬스체크 경로로 사용 가능 |
...
| **미들웨어 순서** | `app/main.py` L56–66 | CORS → `security_headers` → `rate_limit` → `access_log` → `request_id` |
