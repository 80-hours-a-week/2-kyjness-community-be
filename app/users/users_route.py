...
async def get_user(user_id: int, authenticated_user_id: int = Depends(get_current_user)):
    """내 정보 조회 API"""
    return UsersController.get_user(user_id=user_id, authenticated_user_id=authenticated_user_id)

# 내 정보 수정
@router.patch("/{user_id}", status_code=200)
async def update_user(user_id: int, user_data: UpdateUserRequest, authenticated_user_id: int = Depends(get_current_user)):
    """내 정보 수정 API"""
...
async def update_password(user_id: int, password_data: UpdatePasswordRequest, authenticated_user_id: int = Depends(get_current_user)):
    """비밀번호 변경 API"""
...
async def withdraw_user(user_id: int, authenticated_user_id: int = Depends(get_current_user)):
    """회원 탈퇴 API"""
    UsersController.withdraw_user(user_id=user_id, authenticated_user_id=authenticated_user_id)
