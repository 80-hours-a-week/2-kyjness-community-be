...
async def create_comment(comment_data: CommentUpsertRequest, post_id: int = Path(..., description="게시글 ID"), user: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_comments(post_id: int = Path(..., description="게시글 ID"), page: int = Query(1, ge=1, description="페이지 번호"), size: int = Query(10, ge=1, le=100, description="페이지 크기"), db: Session = Depends(get_db)):
...
async def update_comment(comment_data: CommentUpsertRequest, author_ctx: CommentAuthorContext = Depends(require_comment_author), db: Session = Depends(get_db)):
...
async def withdraw_comment(author_ctx: CommentAuthorContext = Depends(require_comment_author), db: Session = Depends(get_db)):
